...
_mesh_serve_pubkey() {
  ...
  while true; do
    ncat -l "${MY_IP}" "${LDOWN_PORT}" --send-only < "${pubfile}" 2>/dev/null
  done &
  echo $!
}
...
cmd_mesh_start() {
  ...
  for i in $(_mesh_sorted_peer_indices); do
    ...
    # wake tunnel — WireGuard won't handshake until traffic is sent
    ping -c1 -W1 "${peer_tunnel}" &>/dev/null || true
...
cmd_mesh_join() {
  ...
  local peer_list
  peer_list="$(printf '%s\n' "JOIN ${MY_NAME} ${MY_TUNNEL_IP} ${my_pubkey}" \
...
EXPORTED_AT=${ts}"
  ...
  must "encrypt bundle" openssl enc -aes-256-gcm -pbkdf2 -iter 100000 \
    -in  "${tarball}" \
    -out "${bundle_out}"
...
cmd_mesh_import() {
  ...
  must "decrypt bundle" openssl enc -d -aes-256-gcm -pbkdf2 -iter 100000 \
    -in  "${bundle}" \
    -out "${tarball}"
...
cmd_mesh_doctor() {
  ...
  if [[ ! -f "${MESH_CONF}" ]]; then
    ...
    (( errors++ )) || true
    # nothing else can run without mesh.conf
    printf '\n'
  ...
  # compare roster hash to what was recorded at init time
  ...
  # only query wg if interface is up
  local wg_peers=""
  "${iface_up}" && \
    wg_peers="$(wg show "${WG_INTERFACE}" latest-handshakes 2>/dev/null || true)"
...
cmd_mesh_neighbors() {
  ...
  for i in $(_mesh_sorted_peer_indices); do
    ...
    if ping_output="$(ping -c3 -W1 -q "${peer_tunnel}" 2>/dev/null)"; then
      # extract avg from: rtt min/avg/max/mdev = 0.123/0.456/0.789/0.012 ms
      local avg_ms
